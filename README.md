...
- 单个文件会合并多个有效来源的命中范围，同一内容只计一次，因此能够识别一份文件分别复用多份来源的情况。
- 作业项列表分别显示同批次来源和参考库来源的最高文件相似度，并支持按两类数值升序、降序或按名称排序。
- 作业项详情分别展示全部文件、代码文件和文档文件中的最高相似度；同批次来源与参考库来源独立计算，不生成混合总分。
...
三种算法均以文本块建立倒排索引，并按文件级 DF 计算 IDF。IDF 的背景语料包含当前类别下的批次文件和参考库文件，用于识别跨来源出现的许可证、模板与公共词组；候选检索和最终结果仍严格区分同批次来源与参考库来源。精确比较只让至少连续 8 个 Token、且累计 IDF 达到 3 个语料唯一特征等价信息量的匹配链生成命中区间；来源还需达到 10% 的 IDF 加权查询覆盖才会进入结果，不再以原始 Token 覆盖率承担去噪。文件相似度先合并同一来源类别下所有命中范围，再计算去重覆盖率；作业项只展示其中最高的文件、代码文件和文档文件相似度。
